"""
Model V4: Fire Prediction with Spatial Autocorrelation
=======================================================
Builds on V3 by adding spatial neighbor features:
  - fwi_vecinos_mean: avg FWI of 8 adjacent cells on the same day
  - fire_vecinos_3d:  any fire in adjacent cells in last 3 days
  - fwi_vecinos_max:  max FWI of adjacent cells on the same day

Usage:
    python model_v4/train_model_v4.py

Expected time: ~15-25 minutes on a modern CPU (spatial joins add overhead).
"""
...
OUT_DIR   = Path(r"D:\Prueba técnica\fire_prediction_model\02_ml_model\model_v4")
...
N_SPARROWS          = 15
N_ITER_SSA          = 15
GRID_RES            = 0.25  # Grid resolution in degrees
...
# 
def add_features(df):
    """Add interaction features + spatial features."""
    ...
    # Interaction: wind × FWI
    df["wind_x_fwi"] = df["wind_speed_10m"] * df["fwi"]

    # NDVI anomaly (deviation from historical mean per cell)
    ...
    n_new = 7  # 4 interactions + 3 spatial
...
def main():
    ...
    # 2. Feature engineering (interactions + spatial)
    df = add_features(df)
    ...
    print("=" * 70)
    print(f"  {'Metric':<25} {'V3':>12} {'V4':>12} {'Delta':>12}")
    print(f"  {'-'*25} {'-'*12} {'-'*12} {'-'*12}")
    ...
    print(f"  {'AUC-ROC':<25} {v3_auc:>12.4f} {auc:>12.4f} {auc-v3_auc:>+12.4f}")
    print(f"  {'Avg Precision':<25} {v3_ap:>12.4f} {ap:>12.4f} {ap-v3_ap:>+12.4f}")
    print(f"  {'Precision (F1-opt)':<25} {v3_prec:>12.4f} {metrics_f1['precision']:>12.4f} "
          f"{metrics_f1['precision']-v3_prec:>+12.4f}")
    print(f"  {'Recall (F1-opt)':<25} {v3_rec:>12.4f} {metrics_f1['recall']:>12.4f} "
          f"{metrics_f1['recall']-v3_rec:>+12.4f}")
    print(f"  {'False Positives':<25} {v3_fp:>12,} {metrics_f1['fp']:>12,} "
