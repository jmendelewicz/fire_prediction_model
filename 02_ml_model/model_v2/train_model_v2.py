...
def add_features(df):
    ...
    df["ndvi_mean"] = df.groupby("cell_id")["ndvi"].transform("mean")
    df["ndvi_anomaly"] = df["ndvi"] - df["ndvi_mean"]
...
def ssa_optimize(X_train, y_train):
    ...
    print(f"\n SSA finished in {(time.time()-t0)/60:.1f} min")
...
def train_final_model(best_params, X_train, y_train, X_test, y_test):
    ...
    print("\n   Fitting final model on full balanced train set...")
